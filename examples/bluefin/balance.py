...
--Requirements--
1) An unbalanced Bluefin 1.5kWh battery.
2) A RS485 cable for communicating with the battery.
3) PySerial (pip3 install pyserial)
    
Note: Battery balancing can take several days depending on the severity of
the misbalance. One should fully charge a battery, balance it, charge it again,
and then balance one more time before deployment.

Author: Ian Black (ian.black@oregonstate.edu)
Date: 2021-07-05
'''
...
def main():
    ...
    print('Connected to battery {}!'.format(sn))
    if sbm.is_balanced(delta=0.030):
        print('Battery already appears to be well balanced. Exiting utility.')
        for i in range(3):
            sbm.off()
            exit()
    print('Initializing Log...')
    with open("SBM{}_{}.txt".format(sn,today),"w") as log:
        log.write("Bluefin 1.5kWh {} Balance Log\n".format(sn))
        log.write("\n\n\n")
        log.write("ISO8601,voltage_array,temperature\n")
        start = time.time()
        while True:
            ...
            if sbm.get_temperature() > 40: #If the battery gets too hot, stop.
                ...
                if sbm.is_balanced(0.030):
                    print('battery is balanced.')
                    sbm.off()
                    exit()
                else:
                    print('battery is not balanced.')
                loop_stop = time.time()
                wait_time = int(60-(loop_stop-loop_start))
                if wait_time < 0:
                    continue
                else:
                    print('Waiting {} seconds before checking again.'.format(wait_time))
...
class SERCOM():
    ...
    def _buffer_check(self,check=0.1):
