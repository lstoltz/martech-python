...
class THETIS():
    ...
    def make_directory(self,directory_id):
        if len(str(directory_id)) > 8:
            ...
            return False
        self.rs232.write_command('$PWETC,PC,,,,MKD,1,.\{}*'.format(directory_id),EOL='\n')
        response = self.rs232.read_response()
        pattern = '\$PWETA,.*?,.*?,.*?,.*?,(.*?)\*'
        created = re.findall(pattern,response).pop()
        if created == 'MKD':
            return True
        else:
            return False
    ...
    def change_directory(self,directory_id):
        self.rs232.write_command('$PWETC,PC,,,,CD,1,.\{}*'.format(directory_id),EOL='\n')
        response = self.rs232.read_response()
        pattern = '\$PWETA,.*?,.*?,.*?,.*?,.*?,.*?,(.*?)\*'            
    ...
    def logging(self,state):
        if state == 'ON':
            self.rs232.write_command('$PWETC,PC,,,,LOG,1,1*0A',EOL='\n')
        elif state == 'OFF':
            self.rs232.write_command('$PWETC,PC,,,,LOG,1,0*0B',EOL='\n')
        response = self.rs232.read_response()
        pattern = '\$PWETA,.*?,.*?,.*?,.*?,.*?,.*?,(.*?)\*'
    ...
    def power_winch(self,state):
        if state == 'ON':
            self.rs232.write_command('$PWETC,PC,,,,WP,1,1*49',EOL = '\n')
        elif state == 'OFF':
            self.rs232.write_command('$PWETC,PC,,,,WP,1,0,*48',EOL = '\n')
        response = self.rs232.read_response()
        if "ON" in response:
    ...
    def power_sensors(self,state):     
        if state == 'ON':
            self.rs232.write_command('$PWETC,PC,,,,CTDP,1,1*4D',EOL='\n')
            ctd = self.rs232.read_response()
            self.rs232.clear_buffers()
            time.sleep(0.5)
            self.rs232.write_command('$PWETC,PC,,,,INSP,1,1*4A',EOL='\n')
            insp = self.rs232.read_response()
            self.rs232.clear_buffers()
            time.sleep(5)
            if "ON" in ctd and "ON" in insp:
                print('Sensors are now on!')
        elif state == 'OFF':
            while True:  #Keep trying to turn off the sensors until successful.
                self.rs232.write_command('$PWETC,PC,,,,CTDP,1,0*4C',EOL='\n')
                ctd = self.rs232.read_response()
                self.rs232.clear_buffers()
                time.sleep(0.5)
                self.rs232.write_command('$PWETC,PC,,,,INSP,1,0*4B',EOL='\n')
                insp = self.rs232.read_response()      
                self.rs232.clear_buffers()
                time.sleep(0.5)
                if "OFF" not in ctd and "OFF" not in insp:
                    time.sleep(1)
                    continue
                elif "OFF" in ctd and "OFF" in insp:
                    print('Sensors are now off!')
                    break        
 
    def power_pump(self,state):
        if state == 'ON':
            self.rs232.write_command('$PWETC,PC,,,,PWR,2,PMP,1*79',EOL='\n')
        elif state == 'OFF':
            self.rs232.write_command('$PWETC,PC,,,,PWR,2,PMP,0*78',EOL='\n')
        response = self.rs232.read_response()
        if "ON" in response:
    ...
    def get_ctd_depth(self):
        self.rs232.write_command('$PWETC,PC,,,,D*0A',EOL='\n')
        response = self.rs232.read_response()
        pattern = '\$PWETC,.*?,.*?,.*?,.*?,D,1,(.*?)\*'
    ...
    def get_surface_switch_status(self):
        self.rs232.write_command('$PWETC,PC,,,,PSW*1A',EOL='\n')
        response = self.rs232.read_response()
    ...
    def get_version(self):
        ...
        print(response)
        pattern = '\$PWETA,.*?,.*?,.*?,(.*?)\*'
        ver_str = re.findall(pattern,response).pop()
        ver_array = ver_str.split(',')
        pro_id = ver_array[0][-1]
        cf_sn = ver_array[3]
        pico = ver_array[4]
        bios = ver_array[5]
        fw = ver_array[6]
        fw_date = ver_array[7]
        ver_array = [pro_id,fw,fw_date,pico,bios,cf_sn]
        return ver_array
    
    def enter_passthru(self,port):
        self.rs232.write_command('$PWETC,PC,,,,pas,1,{}*'.format(port),EOL='\n')
        time.sleep(1)
