'''A module for driving the Sea-Bird Scientific ECO Triplet-w.

Note: Hard coded wait times were determined using a RS232 sniffer.
    Each command was run multiple times and the maximum value plus a small
    buffer was used.
...
class TRIPLETW():
    def __init__(self,port):
        '''Instantiate a serial object with the assigned port.
        In theory, this should allow for the connection of multiple sensors
        of the same type on different ports so long as each instance 
        is defined separately.

        The ECO Triplet-w operates at a user set baudrate (default is 19200)
        with 8N1.
        
        Example:
            sensor1 = TRIPLETW('COM3')
            sensor1.open_connection(19200)
            sensor2 = TRIPLETW('COM4')
            sensor2.open_connection(115200)
        '''
        self.rs232 = RS232()
        self.port = port
        self.bytesize = 8
    ...
    def open_connection(self,baudrate=19200):
        '''Open a connection to the sensor at the defined baudrate.
        baudrate -- bits per second, default for the ECO Triplet-w is 19200.
        return -- true if connection is successful, false if not successful.
        '''
    ...
    def close_connection(self):
        ...
        return -- true if disconnection is successful, false if not successful.
        '''
    ...
    def log_data(self,state="ON"):
        '''Tells the ECO Triplet-w to disable or enable
            recording of data.
        enable -- If True, sensor records data. If False,
            sensor does not record data.
        return -- The state of the Rec setting. If it is
            enabled, the return will be '1'. If it is
            disabled, the return will be '0'.            
        '''
        if state == "ON":
            val = 1
